...
fn focus_reaches_inner_view() {
    ...
    let mut harness =
        FocusTestHarness::new(geometry_reader_single_button, state, Size::new(100, 100))
...
fn navigate_through_inner_stack() {
    let state = State { a: 0, b: 0, c: 0 };
    let mut harness = FocusTestHarness::new(geometry_reader_with_stack, state, Size::new(100, 100))
        .with_roles(Role::Button);
...
fn no_focusable_returns_deferred() {
    let state = State { a: 0, b: 0, c: 0 };
    let mut harness =
        FocusTestHarness::new(geometry_reader_no_focusable, state, Size::new(100, 100))
            .with_roles(Role::Button);
...
fn geometry_reader_in_stack_forward() {
    let state = State { a: 0, b: 0, c: 0 };
    let mut harness = FocusTestHarness::new(geometry_reader_in_stack, state, Size::new(100, 100))
        .with_roles(Role::Button);
...
fn nested_geometry_readers_focus() {
    let state = State { a: 0, b: 0, c: 0 };
    let mut harness = FocusTestHarness::new(nested_geometry_readers, state, Size::new(100, 100))
        .with_roles(Role::Button);
