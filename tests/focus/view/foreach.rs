//! Tests for focus navigation within `ForEach` views.
//!
//! Note `ForEach` should probably require an id from its subviews
//! to avoid issues when the data changes. But that doesn't exist yet
...
fn first_item_is_focusable() {
    let mut harness =
        FocusTestHarness::new(foreach_three_items, State::default(), Size::new(100, 300))
...
fn empty_list_returns_deferred() {
    let mut harness = FocusTestHarness::new(foreach_empty, State::default(), Size::new(100, 100))
        .with_roles(Role::Button);
...
fn single_item_list_navigation() {
    let mut harness =
        FocusTestHarness::new(foreach_one_item, State::default(), Size::new(100, 100))
            .with_roles(Role::Button);
...
fn stack_navigation_through_foreach() {
    let mut harness =
        FocusTestHarness::new(stack_with_foreach, State::default(), Size::new(100, 300))
            .with_roles(Role::Button);
