...
fn some_is_focusable() {
    ...
    let mut harness =
        FocusTestHarness::new(view_with_some, state, Size::new(100, 100)).with_roles(Role::Button);
...
fn none_skips_to_next() {
    ...
    let mut harness =
        FocusTestHarness::new(view_with_none, state, Size::new(100, 100)).with_roles(Role::Button);
...
fn none_returns_deferred() {
    ...
    let mut harness =
        FocusTestHarness::new(view_only_none, state, Size::new(100, 100)).with_roles(Role::Button);
