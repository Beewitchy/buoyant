...
fn first_variant_is_focusable() {
    ...
    let mut harness =
        FocusTestHarness::new(two_branch_view, state, Size::new(100, 100)).with_roles(Role::Button);
...
fn select_triggers_action_on_correct_variant() {
    ...
    let mut harness = FocusTestHarness::new(three_branch_view, state, Size::new(100, 100))
...
fn unfocusable_variant_returns_deferred() {
    ...
    let mut harness =
        FocusTestHarness::new(view_with_unfocusable_branch, state, Size::new(100, 100))
            .with_roles(Role::Button);
...
fn stack_navigation_through_match_view() {
    ...
    let mut harness = FocusTestHarness::new(stack_with_match_view, state, Size::new(100, 100))
        .with_roles(Role::Button);
    ...
    harness.select();
    assert_eq!(harness.state.b, 1, "Rectangle button action should trigger");
}
...
fn focus_backward_on_match_view() {
    ...
    let mut harness = FocusTestHarness::new(two_branch_view, state, Size::new(100, 100))
        .with_roles(Role::Button)
        .with_focus_at_end();
...
fn variant_change_while_focused_first_to_second() {
    ...
    harness.state.variant = Variant::Second;
    harness.rebuild();
...
fn variant_change_while_focused_second_to_first() {
    ...
    harness.state.variant = Variant::First;
    harness.rebuild();
...
fn variant_change_cycles_through_all_three() {
    ...
    harness.state.variant = Variant::Third;
    harness.rebuild();
