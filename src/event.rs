...
use crate::{
    focus::{FocusAction, FocusGroup, RoleSet},
    primitives::Point,
    render::ContentShape,
};
...
impl Event {
    ...
    pub fn offset(&self, offset: Point) -> Self {
        ...
        match &mut event {
            ...
            Self::Scroll(_) | Self::Focus(_) | Self::KeyDown(_) | Self::KeyUp(_) => {}
        }
        event
    }
}
...
impl EventResult {
    ...
    /// Returns true if this result represents a focused element.
    #[must_use]
...
pub struct EventContext {
    ...
    pub roles: RoleSet,
    /// Which focus group is currently active.
...
impl EventContext {
    ...
    pub const fn new(app_time: Duration) -> Self {
        Self {
            ...
            roles: RoleSet::any(),
            focus_group: FocusGroup::common_group(),
        }
    }
    ...
    /// Creates a new `EventContext` with the specified focus group.
    #[must_use]
