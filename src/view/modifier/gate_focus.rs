...
/// Requests using the common group are always passed through.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GateFocus<T> {
    child: T,
    group: FocusGroup,
}

impl<T: ViewMarker> GateFocus<T> {
    /// Creates a new `GateFocus` modifier that gates focus for the given group index.
    #[must_use]
...
impl<T: ViewMarker> ViewMarker for GateFocus<T> {
    type Renderables = T::Renderables;
    type Transition = T::Transition;
}

impl<Captures: ?Sized, T: ViewLayout<Captures>> ViewLayout<Captures> for GateFocus<T>
where
    T::FocusTree: DefaultFocus,
{
    ...
    fn handle_event(
        ...
        if context.focus_group == self.group {
            return self
                .child
                .handle_event(event, context, render_tree, captures, state, focus)
                .with_group(self.group);
