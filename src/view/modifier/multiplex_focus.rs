    ...
    T::FocusTree: DefaultFocus,
{
    ...
    fn handle_event(
        ...
        let mut handle_indexed = |index: u8| {
            let group_tree = match event {
                Event::Focus(FocusAction::Focus(d)) => focus.get_or_init(index, *d),
                Event::Focus(FocusAction::Next) => {
                    focus.get_or_init(index, FocusDirection::Forward)
                }
                Event::Focus(FocusAction::Previous) => {
                    focus.get_or_init(index, FocusDirection::Backward)
                }
                Event::Focus(FocusAction::Blur | FocusAction::Select)
                | Event::Scroll(_)
                | Event::KeyUp(_)
                | Event::KeyDown(_) => {
        ...
        if let Some(index) = context.focus_group.index() {
            if index as usize >= N {
                // No matching group found, e.g. ::<N = 5> and context is asking for group 7
                return EventResult::Deferred;
            }

            handle_indexed(index)
        } else {
            // Request is for common group
            // TODO: Is this only right for touch events?
            ...
            if let EventResult::Handled { group, .. } = result
                && let Some(focused_index) = group.index()
                && (focused_index as usize) < N
            {
                focus.0[focused_index as usize] = Some(candidate_focus);
            }
            result
        }
    }
...
/// ```compile_fail
/// use buoyant::view::modifier::MultiplexFocus;
/// _ = MultiplexFocus::<u8, 0>::new();
/// ```
#[expect(unused)]
