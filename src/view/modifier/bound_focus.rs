    ...
    T::FocusTree: DefaultFocus,
{
    ...
    fn handle_event(
        ...
        // FIXME: Needs tests
        let Event::Focus(focus_event) = event else {
            return self
                .child
        ...
        // Focus event was deferred - child couldn't handle it
        // This means we've hit a boundary (or there are no focusable elements)
        ...
        match self.behavior {
            BoundaryBehavior::Wrap => {
                ...
                *focus = if is_forward {
                ...
                let wrap_result = self.child.handle_event(
                ...
                // If wrapping also fails, there are no focusable elements - defer
                if matches!(wrap_result, EventResult::Deferred) {
            ...
            BoundaryBehavior::Stop => {
                ...
                let stop_result = self.child.handle_event(
                ...
                // If refocusing fails, there are no focusable elements - defer
                if matches!(stop_result, EventResult::Deferred) {
                    return EventResult::Deferred;
                }
