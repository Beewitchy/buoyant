...
use crate::{
    primitives::{
        Interpolate, Point,
        geometry::{Intersection, Rectangle, embedded_graphics::DrawProvider},
...
use embedded_graphics::{
    draw_target::DrawTarget, pixelcolor::PixelColor, primitives::PrimitiveStyleBuilder,
};
    ...
    C: PixelColor + Interpolate + AlphaColor,
{
    ...
    fn fill<T: Into<Self::ColorFormat>, S: Shape>(
        &mut self,
            ...
            if let Some(ref active_clip_rect) = self.active_layer.clip_rect {
                match active_clip_rect.intersection_with(&bounding_box) {
                    Intersection::Contains => {
                        ...
                        S::Draw::draw(&mut target, shape, &transform, &style);
                    }
    ...
    fn stroke<T: Into<Self::ColorFormat>, S: Shape>(
        &mut self,
        stroke: &Stroke,
        transform: impl Into<LinearTransform>,
        brush: &impl Brush<ColorFormat = T>,
